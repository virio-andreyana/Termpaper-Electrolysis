...
seq_lim = 0.1
co2pipe_cost = 0.04
EmissionRatio, SequesterRatio = adjust_CO2_capture(cc_share)
InvestmentCost = adjust_investmentcost(cc_cost)
SequesterCost = 0.26 # Cost of sequestering per Ton of CO2
TotalSequesterLimit = limit_regional_sequestering(seq_lim)
...
@variable(ESM,CO2Import[regions,regions] >= 0)
@variable(ESM,CO2Export[regions,regions] >= 0)
...
@constraint(ESM, TotalSequesterAccounting[r in regions],
    sum(Production[y,r,h,t,f] * SequesterRatio[t] * YearlyDifferenceMultiplier[y] for f in fuels, h in hour,y in year,t in technologies) + sum(CO2Import[r,rr] for rr in regions)  == TotalSequester[r] + sum(CO2Export[r,rr] for rr in regions)
)
...
# TODO: NEW: account for the total sequester emission
@constraint(ESM, CO2TradeBalance[r in regions, rr in regions],
    CO2Import[r,rr] == CO2Export[rr,r]
)
...
@objective(ESM, Min,
    ...
    + TotalSequesterCost
    + sum(CO2Export[r,rr]*TradeDistance[r,rr]*co2pipe_cost for r in regions, rr in regions) # TODO: test
)
...
result_path = mkpath(joinpath(@__DIR__, "Results test_trade seq_lim$seq_lim"))
